...
/**
 * NoColorInfluence class contains the method getInfluence utilized to calculate the influence of an island for a
 * single player, ignoring a color.
 *
...
public class NoColorInfluence implements InfluenceStrategy {
    ...
    /**
     * Method getInfluence calculates the influence on the island i, for the player p (given by parameter), ignoring the
    ...
    public int getInfluence(Island i, Player p, Optional<ColorS> color, Map<ColorS, Player> profs) {
        ...
        for(ColorS c : ColorS.values()) {
            if (Optional.ofNullable(profs.get(c)).equals(Optional.of(p)) && !(Optional.of(c).equals(color))) {
